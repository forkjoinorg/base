...
//	}
	...
	private static final void toSqlWhereOpt(StringBuilder sb,QueryParam.OperatorType opt){
		switch (opt) {
		case EQ:
			sb.append(" = ? ");
			break;
	...
	public int setPreparedStatement(PreparedStatement ps,int i) throws SQLException{
		for(QueryParam p:params){
			if(p.opt!= QueryParam.OperatorType.IS_NULL&&p.opt!= QueryParam.OperatorType.IS_NOT_NULL){
