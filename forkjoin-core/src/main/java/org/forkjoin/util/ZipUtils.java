...
import org.forkjoin.core.sql.ByteArrayBlob;
import org.forkjoin.core.io.IoUtils;
...
//        zipBuf.release();
    ...
    public static Blob compress(String data) throws IOException {
        ...
        cout.write(data.getBytes(CHARSET_NAME));
        cout.finish();
        cout.flush();
        cout.close();
        return new ByteArrayBlob(out.getBuf(), out.size());
    }

    public static String decompression(Blob blob) throws IOException, SQLException {
        InputStream binaryStream = blob.getBinaryStream();
        GzipCompressorInputStream in = new GzipCompressorInputStream(binaryStream);

        ByteArrayBuffer out = new ByteArrayBuffer();
        IOUtils.copy(in, out);
        return new String(out.getBuf(), 0, out.size(), CHARSET_NAME);
    }
...
//        BuffBet
    ...
    private static final class ByteArrayBuffer extends ByteArrayOutputStream {
        private byte[] getBuf() {
            return super.buf;
        }
    }
