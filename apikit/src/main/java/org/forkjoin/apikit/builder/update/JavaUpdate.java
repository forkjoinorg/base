...
public class JavaUpdate {
    ...
    private File file;
    private BuilderUtils utils;
    private ApiInfo moduleInfo;


    public JavaUpdate(File file, BuilderUtils utils, ApiInfo moduleInfo) {
        this.file = file;
        this.utils = utils;
        this.moduleInfo = moduleInfo;
    }

    public void update(String targetSource) {
        try {
            String source = FileUtils.readFileToString(file, ENCODING);
            CompilationUnit sourceNode = createCompilationUnit(source);
            TypeDeclaration sourceType = getType(utils.getName(), sourceNode);
            if (sourceType == null) {
                throw new RuntimeException("未找到需要更新文件里面有所需类型！");
            }

            CompilationUnit targetSourceNode = createCompilationUnit(targetSource);
            TypeDeclaration targetSourceType = getType(utils.getName(), targetSourceNode);
            ...
            HttlHelper.writeStringToFile(file, formatCode, ENCODING);
        } catch (IOException e) {
