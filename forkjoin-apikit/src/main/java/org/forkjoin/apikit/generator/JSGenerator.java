 ...
 * 这个版本还不能 ,解开泛型或者支持泛型
 */
public class JSGenerator extends HttlGenerator {
    private JSWrapper.Type type = JSWrapper.Type.FLOW_TYPE;

    protected File getFileName(BuilderWrapper utils) {
        return Utils.packToPath(srcPath, utils.getPack(), utils.getName(), ".js");
    }
    ...
    public void generateTool() throws Exception {
        {
            ...
            File file = Utils.packToPath(srcPath, "", "index", ".js");
...

        {
            ...
            File file = Utils.packToPath(srcPath, "", "Apis", ".js");
            execute(
                    parameters,
